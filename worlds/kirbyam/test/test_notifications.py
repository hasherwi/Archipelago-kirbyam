...
def test_send_notification_includes_sender_and_location():
    """_maybe_emit_send_notification should include sender and location in message."""
    client = KirbyAmClient()
    ...
    if data.items:
        test_item_id = next(iter(data.items.keys()))
    if data.locations:
