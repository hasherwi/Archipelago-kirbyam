...
_MINOR_CHEST_FLAGS_ADDR_KEY = "other_chest_flags_native"
...
class KirbyAmClient(BizHawkClient):
    ...
    def initialize_client(self) -> None:
        ...
        # Other-chest bitfield -> location IDs (OTHER_CHEST category; polled from native small chest flags).
        self._chest_field_location_ids_by_bit = self._build_location_ids_by_bit(LocationCategory.OTHER_CHEST)
        ...
        self._last_chest_field_poll_log: tuple[str, tuple[int, ...], tuple[int, ...]] | None = None
    ...
    def _log_notify(self, level: str, msg: str, *args: object, **kwargs: object) -> None:
        ...
        Use for critical player-facing events requiring immediate attention.
        """
        logger_obj = self._get_logger()
        logger_func = getattr(logger_obj, level, logger_obj.info)
        logger_func(msg, *args, **kwargs)
        # Note: BizHawk popup dispatch happens at call sites to allow for async handling.

    def _log_client(self, level: str, msg: str, *args: object, **kwargs: object) -> None:
        """
        Level 2 (Client): AP client output + log file (no popup).
        Use for important non-critical info and diagnostics.
        """
        logger_obj = self._get_logger()
        logger_func = getattr(logger_obj, level, logger_obj.info)
        logger_func(msg, *args, **kwargs)
    ...
    def _get_logger() -> logging.Logger:
        # Late import avoids world-load circular imports and keeps tests that patch
        # CommonClient.logger observing calls from these helper methods.
    ...
    def _reset_reconnect_transient_state(self) -> None:
        ...
        self._last_chest_field_poll_log = None
    ...
    async def game_watcher(self, ctx: "BizHawkClientContext") -> None:
        ...
        try:
            ...
            # Chest-field location polling via native small chest flag bitfield
            await self._poll_chest_field_locations(ctx)
    ...
    async def _poll_and_send_local_death_link(self, ctx: "BizHawkClientContext") -> None:
        ...
        if self._last_local_alive_state and not alive_now:
            if self._suppress_next_local_death_send:
                ...
                self._log_verbose("info", "KirbyAM: sending outgoing DeathLink with flavored cause")
    ...
    async def _poll_chest_field_locations(self, ctx: KirbyAmBizHawkClientContext) -> None:
        """
        Read native small_chest_flags and map set bits to OTHER_CHEST locations.

        This is the generic chest bitfield discovered in the decomp. It uses the same
        resend/dedupe semantics as the other RAM-derived location pollers.
        """
        ...
        chest_bits = int.from_bytes(raw, "little")
        active_location_ids = self._active_location_id_set(ctx)

        mapped_checked_locations: set[int] = set()
        for bit in sorted(self._chest_field_location_ids_by_bit.keys()):
            if (chest_bits >> bit) & 1:
                mapped_checked_locations.update(self._chest_field_location_ids_by_bit.get(bit, []))
        ...
        if missing_on_server:
            chest_log_state = ("resend", tuple(missing_on_server), tuple(already_acknowledged))
            if chest_log_state != self._last_chest_field_poll_log:
                self._log_verbose(
                    "info",
                    "KirbyAM: resending other-chest LocationChecks missing on server (missing=%s, acked=%s)",
                    missing_on_server,
                    already_acknowledged,
                )
                self._last_chest_field_poll_log = chest_log_state
            ...
            if chest_log_state != self._last_chest_field_poll_log:
                self._log_verbose(
                    "debug",
                    "KirbyAM: dedupe suppressed other-chest LocationChecks (all RAM-derived checks already acknowledged: %s)",
                    already_acknowledged,
                )
                self._last_chest_field_poll_log = chest_log_state
        else:
    ...
    async def _probe_boss_defeat_candidates(self, ctx: KirbyAmBizHawkClientContext) -> None:
        """
        Probe native candidate boss table bytes for diagnostics only.

        Boss-defeat checks are transport-authoritative via boss_defeat_flags.
        Native probe rises are logged to aid low-level debugging and telemetry.
        """
    ...
    async def _deliver_items(self, ctx: KirbyAmBizHawkClientContext, allow_new_writes: bool = True) -> None:
        ...
        if rom_received_count is not None:
            if rom_received_count > len(ctx.items_received):
                if not self._delivery_counter_ahead_fallback_active:
                    self._log_verbose(
                        "info",
                        "KirbyAM: ROM delivery counter is ahead of received items (rom=%s, received=%s); ignoring ROM counter and continuing mailbox delivery",
                        rom_received_count,
                        len(ctx.items_received),
                    )
                ...
                if self._delivery_counter_ahead_fallback_active:
                    self._log_verbose(
                        "info",
                        "KirbyAM: ROM delivery counter is back in range (rom=%s, received=%s); restoring normal mailbox synchronization",
                        rom_received_count,
                        len(ctx.items_received),
                    )
                self._delivery_counter_ahead_fallback_active = False
            ...
            if timeout_triggered:
                ...
                if (
                    ...
                    if hook_heartbeat is not None and self._hook_heartbeat_stale_ticks >= 3:
                        ...
                        self._log_client(
                            "warning",
                            "KirbyAM: Repeated mailbox ACK timeouts with frame_counter stuck at 0; payload hook may be inactive in the loaded ROM patch"
                        )
        ...
        while self._delivered_item_index < len(ctx.items_received):
            ...
            if self._delivery_counter_ahead_fallback_active and not self._delivery_counter_ahead_resume_logged:
                self._log_verbose(
                    "info",
                    "KirbyAM: ROM counter fallback active; continuing mailbox delivery at item index %s (rom=%s, received=%s)",
                    self._delivered_item_index,
                    rom_received_count,
                    len(ctx.items_received),
                )
    ...
    def _hidden_area_boss_goal_signal_active(self, ctx: KirbyAmBizHawkClientContext, hidden_goal_key: str) -> bool:
        """Return whether the selected hidden area boss has been acknowledged by the server."""
        hidden_goal = data.locations.get(hidden_goal_key)
        if hidden_goal is None or hidden_goal.location_id is None:
            return False
    ...
    async def _maybe_report_goal(
        ...
        if clamped_goal:
            self._log_client(
                "warning",
                "KirbyAM: unexpected slot goal value '%s' (parsed=%s); clamping to Dark Mind. This may indicate mismatched configs or world versions.",
                slot_goal_raw,
                parsed_slot_goal,
            )
        ...
        if slot_goal == Goal.option_defeat_random_hidden_area_boss:
            ...
            if goal_location_id is None or hidden_goal_key is None:
                self._log_client(
                    "warning",
                    "KirbyAM: hidden goal target missing or invalid; goal reporting disabled.",
                )
                return
        ...
        if goal_location_id in ctx.checked_locations:
            from NetUtils import ClientStatus
            self._log_verbose("info", "KirbyAM: goal complete; sending CLIENT_GOAL status (goal_option=%s)", slot_goal)
            await self._display_client_message(ctx, "Goal complete")
            await ctx.send_msgs([{"cmd": "StatusUpdate", "status": ClientStatus.CLIENT_GOAL}])
            ctx.finished_game = True
            self._goal_reported = True
