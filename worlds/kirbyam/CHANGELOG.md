...
- Add completely-random per-swallow reroll telemetry logging (`Kirby swallowed a <EnemyName>. Ability was rerolled to <AbilityName>.`) to log files on every runtime event, while only streaming that line to the live client when `enable_debug_logging` is enabled.
