...
- Add `Starting Kirby Color` (`starting_kirby_color`) with canonical color definitions in `data/colors.json`, generation-time deterministic `random_color` resolution, resolved color emission in `slot_data` (`starting_kirby_color` + `starting_kirby_color_name`), and runtime payload synchronization via new mailbox register `starting_kirby_color_id` (`0x0203B050`) that applies non-Pink palettes while keeping Pink as no-op behavior. Generation/client logs now always record the selected color in log files and only stream those messages when `enable_debug_logging` is enabled (Issue #597).
