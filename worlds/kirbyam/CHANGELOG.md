...
- Added a dedicated boolean option surface for ability statues (sometimes called ability trophies or ability stands) via `ability_randomization_statues`; it controls inclusion only, and enabled statues inherit the selected enemy copy-ability mode (`off`, `shuffled`, `completely_random`) (Issue #209).
