...
- Improve sent item notifications to include detailed information (Issue #432): notifications now display format `"<sender_name> sent <item_name> to <receiver_name> (<location_name>)"` with graceful fallback when location context unavailable. Item names resolve from world item data, location names from AP location address mappings, and player names from AP context or player ID fallback.
