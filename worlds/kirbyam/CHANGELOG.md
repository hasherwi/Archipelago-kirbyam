...
- Improve `worlds/kirbyam/build.py` usability for non-author machines by prompting for missing required patch inputs when `--source-type arg` is selected, failing fast with actionable `rom_path.tmp` guidance for `--source-type file`, and introducing `--non-interactive` fail-fast behavior for automation/CI (Issue #607).
...
- Harden vitality delivery against transition/reset replay by clamping native vitality counter grants to the shipped AP vitality item cardinality (4), preventing over-grants even if mailbox/session replay guard state is reset mid-run (Issue #571).
...
- Add `Randomize Non-Ability Enemies` world option (`ability_randomization_passive_enemies`) that, when enabled alongside non-off enemy copy-ability randomization, allows enemies that normally grant no copy ability (e.g. Waddle Dee, Bronto Burt, Batty) to receive a randomized ability from the whitelist. Includes 16 additional enemies and a split Waddle Dee mini-boss entry (`WADDLE_DEE_MINI`) gated by the existing mini-boss toggle. The shipped default is `true` (Issue #398).
