...
- Added a dedicated boolean option surface for statue/stand copy-ability randomization (`ability_randomization_statues`) that controls inclusion only; when enabled, statues inherit the selected enemy copy-ability mode (`off`, `shuffled`, `completely_random`) (Issue #209).
