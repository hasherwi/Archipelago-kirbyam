...
- Expanded exploration location check coverage:
  ...
  - The first time you visit each area is now a location check (Issue #606). These nine new location check checks give us room to add progression gating items later. This is not a toggleable feature and is separate from Room Sanity.
...
- Chests:
  - Minor (small) chests are labelled very poorly right now. We need the community to help us label them correctly because no mapping currently exists for this "bit" maps to this chest and this item in the chest.
  - The items inside minor (small) chests are not AP given items right now. If you open a chest with a spray paint, the game will give you that spray paint AND AP sends a location check. This is expected. We want to get the locations working, before we got to interrupting game item delivery.
